...
//     // locations
//     const {data: locationData, isSuccess: locationSuccess} = UseFetchLocationQuery({detailed:false})
//     useEffect(()=>{
//         if(locationData?.results && locationSuccess){
//             const results: any[] = locationData?.results
//             const simplifiedLocations = results.map(({ id, name }: any) => ({ id, name }));
//             console.log(simplifiedLocations)
//         }
...
//                                     <CustomDropdown items={items} selectedItem={selectedItem} onSelect={setSelectedItem}/>
//                                 </>
//                             )}
//                         </>
//                     )}
...
const Auth = () => {
    ...
    const [loginUser, { data: loginResults, isSuccess: loginSuccess, error: loginErrorData, isLoading: loginLoading, isError: loginError }] = UseLoginUserMutationL()
    
    useEffect(() => {
        if (loginResults && loginSuccess) {
            if (loginResults?.error) {
                dispatch(addToast({ id: Date.now(), message: loginResults.error, type: 'error' }));
            } else if (loginResults?.results) {
                dispatch(addToast({ id: Date.now(), message: 'Logged in successfully!', type: 'success' }));
                navigate('/')
            }
        }
    ...
    }, [loginResults, loginSuccess, loginError])

    // locations
    const { data: locationData, isSuccess: locationSuccess } = UseFetchLocationQuery({ detailed: false })
    const [simplifiedLocations, setSimplifiedLocations] = useState([])

    useEffect(() => {
        if (locationData?.results && locationSuccess) {
            const results = locationData?.results.map(({ id, name }: any) => ({ id, name }));
            console.log(locationData.results);
            setSimplifiedLocations(results);
        }
    }, [locationSuccess, locationData])
                    ...
                    {login ? (
                        ...
                        <>
                            ...
                            {currentStep === 3 && (
                                <>
                                    <InputDiv label="Password" type="password" name="password" value={registerInfo.password} onChange={handleChange} />
                                    <InputDiv label="Confirm Password" type="password" name="password" value={registerInfo.password} onChange={handleChange} />
                                </>
                            )}
                            {
                                currentStep === 4 &&(
                                    <>
                                        <CustomDropdown
                                            options={simplifiedLocations}
                                            value={registerInfo.location_id}
                                            onChange={(id: any) => setRegisterData({ ...registerInfo, location_id: id })}
                                        />
